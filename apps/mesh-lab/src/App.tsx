...
export function App() {
  const [composition, setComposition] = useState(INITIAL_SEED);
  const [volume, setVolume] = useState(INITIAL_VOLUME);
  const mesh = useMeshAudio(INITIAL_SEED, INITIAL_VOLUME);
  ...
  const changeComposition = (value: number) => {
    setComposition(value);
    mesh.setSeed(value, volume);
  };
